...
let input=`position=<-31684, -53051> velocity=< 3,  5>
...
function advance(input) {
    ...
    const points = lines.map(line => {
        const point = line.substring(line.indexOf('<') + 1, line.indexOf('>'));
        return point.split(', ').map(str => parseInt(str));
    });
    const velocities = lines.map(line => {
        const vel = line.substring(line.lastIndexOf('<') + 1, line.lastIndexOf('>'));
